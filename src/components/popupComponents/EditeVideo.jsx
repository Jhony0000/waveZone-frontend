...
import { editeVideo } from "../../backend/videoApi/videoApi";
import { useSelector } from "react-redux";
import { ID } from "appwrite";

function EditeVideo({ onClose, videoId }) {
  const { register } = useForm();
  const [title, gettitle] = useState("");
  const [titleLength, setTitleLength] = useState(title.length);
  const userData = useSelector((state) => state.auth.userData);

  const eaditeVideo = async () => {
    try {
      await editeVideo({ id: videoId, title: title });
      onClose();
    } catch (error) {
      console.log("Eadite video page Error : ", error);
    }
  };
      ...
      <div className="popup-overlay">
        <div className="popup-content">
          ...
          <div className="user-uplod-detels mt-3">
            ...
            {title.length <= 200 && (
              <button className="post-from-btn" onClick={eaditeVideo}>
                Submit
              </button>
            )}
          </div>
